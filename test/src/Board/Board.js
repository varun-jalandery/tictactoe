...
describe('src/Board/Board', () => {
    it('Board of size 5 should be constructed with 25 cells', done => {
        ...
        assert.equal(
            board.getNumberOfCells(),
            25,
            'getNumberOfCells() should return 25'
        );
        done();
    });

    it('Board should not be constructed with illegal size', done => {
        expect(() => new Board('ILLEGAL_SIZE')).to.throw(
            'size is not valid, "ILLEGAL_SIZE"'
        );
        done();
    });
    ...
    it('getCellCoordinate() should return correct coordinates', done => {
        ...
        assert.equal(
            coordinate.row,
            0,
            'cell number 1 should have row equal to 1'
        );
        assert.equal(
            coordinate.col,
            0,
            'cell number 1 should have col equal to 1'
        );
        ...
        assert.equal(
            coordinate.row,
            2,
            'cell number 14 should have row equal to 2'
        );
        assert.equal(
            coordinate.col,
            3,
            'cell number 14 should have row equal to 3'
        );
        ...
        assert.equal(
            coordinate.row,
            4,
            'cell number 25 should have row equal to 4'
        );
        assert.equal(
            coordinate.col,
            4,
    ...
    it('getCellNumber() should return correct cellNumber', done => {
        ...
        assert.equal(
            cellNumber,
            14,
            'row 2, col 3 should return cell number 14'
        );
        ...
        assert.equal(
            cellNumber,
            25,
            'row 4, col 4 should return cell number 25'
        );
    ...
    it('getCell() should return the correct cell', done => {
        ...
        assert.equal(
            cellFifteen.getValue(),
            15,
            'cell[2][4] should have value 15'
        );
        assert.equal(
            board.getCell(26),
            false,
            'cell 26 does not exist and getCell() should return false'
        );
        done();
    });
    ...
    it('markCell() should mark cells correctly', done => {
        ...
        assert.equal(
            board.markCell(26, 'x'),
            'No such cell number : 26',
            'markCell(26) should not mark non-existing cells'
        );
        assert.equal(
            board.markCell(25, 'x'),
            'Cell number : 25 is already marked',
            'markCell(25) should not mark occupied cells'
        );

        assert.equal(
            board.getCell(1).getValue(),
            'x',
            'cell 1 should have symbol x'
        );
        assert.equal(
            board.getCell(14).getValue(),
            'o',
            'cell 1 should have symbol o'
        );
        assert.equal(
            board.getCell(25).getValue(),
            'x',
            'cell 1 should have symbol x'
        );
        done();
    ...
    it('isBoardFull() should return true if all cells have been occupied by players', done => {
        ...
        assert.equal(
            board.isBoardFull(),
            true,
            '3x3 board should be full after 9 moves.'
        );
        done();
    });
    ...
    it('getNumberOfCellsOccupied() should return n after n moves', done => {
        ...
        assert.equal(
            board.getNumberOfCellsOccupied(),
            7,
            'After seven moves, number of occupied cells should be 7'
        );
        done();
    });
});
